...
my %genome = getGenome($input_table); #get a list of genomes to be annotated
print "######################################\n";
foreach my $genome (keys %genome)
{
    ...
    my $db = "/share/database/ARDB/ardbAnno1.0/blastdb/blastdb/resisGenes.pfasta";
...
#    system("blastall -p $program -d $db -i $seqfile -e $evalueCutoff -b 3 -v 3 -o $outfile");
     system ("$program -db $db -query $seqfile -evalue $evalueCutoff -num_descriptions 3 -num_alignments 3 -num_threads 2 -out $outfile");
}
print "######################################\n\n";
