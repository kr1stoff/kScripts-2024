...
my ($list, $nr,$eggNOG, $swissprot, $cog, $kegg, $go, $obo, $interpro, $outxls, $outstat, $help);
GetOptions (
	"list:s" => \$list,
	"nr:s" => \$nr,
	"eggNOG:s" => \$eggNOG,
...
if ( !$list || (!$nr && !$eggNOG && !$go && !$cog && !$kegg && !$swissprot && !$interpro) || $help || ($go && !$obo)) {
	die <<USAGE;
=========================================================================================
...
Options:
  * -list           input all_gene_id.list, no header, format: <geneID  somethingOrnot>
	...
	-eggNOG			input eggNOG annotation file
...
if ($eggNOG){
	phrase_eggNOG(\%info,$eggNOG,"eggNOG");
	push @db,"eggNOG";
}
if ($kegg) {
	phrase_KEGG(\%info,$kegg,"KEGG");
	push @db,"KEGG";
}
if ($go) {
...
sub phrase_eggNOG {
	my ($hash, $file, $key) = @_;
	open ANNO, $file or die $!;<ANNO>;
	while (<ANNO>) {
		next if (/^#/ || /^\s*$/);
		chomp; my @a = split /\t/,$_;
		$$hash{$a[0]}{$key} ||= $a[21]; # description
	}
	close ANNO;
}
